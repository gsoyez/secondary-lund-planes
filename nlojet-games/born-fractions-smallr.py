...
with PdfPages(f'born-fractions-smallr.pdf') as pdf:
    ...
    for selection in ["dijet", "groom"]:
        ...
        for v,l in zip(all_vars, all_labs):
            ...
            y2=0.045
            if selection=="dijet":
                ax.text(0.05, y1, r'anti-$k_t(R=0.2)$, $p_{t,{\rm lead}}>700$ GeV, $|y|<1.7$',  ha='left', va='baseline', transform=ax.transAxes, fontsize=9, color='black')

                if v=="ptmin":
                    ax.text(0.05, y2, r'$p_{t,{\rm sublead}}<200$ GeV, $0.4<\Delta R<0.6$',    ha='left', va='baseline', transform=ax.transAxes, fontsize=9, color='black')
                elif v=="ptmax":
                    ax.text(0.05, y2, r'$150<p_{t,{\rm sublead}}$ GeV, $0.4<\Delta R<0.6$',   ha='left', va='baseline', transform=ax.transAxes, fontsize=9, color='black')
                else:
                    ax.text(0.05, y2, r'$150<p_{t,{\rm sublead}}<200$ GeV, $\Delta R<0.6$', ha='left', va='baseline', transform=ax.transAxes, fontsize=9, color='black')
            else:
                ax.text(0.05, y1, r'anti-$k_t(R=0.4)$, $p_{t,{\rm lead}}>1$ TeV, $|y|<1.7$',   ha='left', va='baseline', transform=ax.transAxes, fontsize=9, color='black')

                if v=="zmin":
                    ax.text(0.05, y2, r'$z_g<0.2$, $\Delta R|y|>0.3$', ha='left', va='baseline', transform=ax.transAxes, fontsize=9, color='black')
                elif v=="zmax":
                    ax.text(0.05, y2, r'$0.1<z_g$, $\Delta R|y|>0.3$', ha='left', va='baseline', transform=ax.transAxes, fontsize=9, color='black')
                else:
