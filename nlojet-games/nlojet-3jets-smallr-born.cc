...
void UserHHC::initfunc(unsigned int){
  ...
  ostringstream oss;
  if (pdf_mem>0){
    oss << "output/parts/3j-smallr-" << pdfname << "_" << pdf_mem << "-" << level << "-iseq" << iseq << ".ohist";
  } else {
    oss << "output/parts/3j-smallr-" << pdfname << "-" << level << "-iseq" << iseq << ".ohist";
  }
  __ohistfile = oss.str();
  ...
  for (unsigned int iscale=0; iscale<mufs.size(); ++iscale){
    ...
    histograms[(nhist*iscale+dijet_tot)*3+2].declare("sigma_dijet_v_drmin"     +oss.str(), -0.6, -0.0, 12);
    ...
    histograms[(nhist*iscale+dijet_g  )*3+2].declare("sigma_dijet_g_v_drmin"   +oss.str(), -0.6, -0.0, 12);
    ...
    histograms[(nhist*iscale+groom_tot)*3+2].declare("sigma_groom_v_drmin"     +oss.str(), -0.4, -0.0 ,  8);
    ...
    histograms[(nhist*iscale+groom_g  )*3+2].declare("sigma_groom_g_v_drmin"   +oss.str(), -0.4, -0.0 ,  8);
